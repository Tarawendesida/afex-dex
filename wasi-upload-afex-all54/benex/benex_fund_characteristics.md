# BENEX Fund Characteristics

## Status

- Stage: `proposed_pre_launch`
- Detail level: `starter_profile`
- Scope: `54 sovereign African states build`
- Country: `Benin`

## Core Identity

- Fund name: `BENEX Fund`
- Full name: `Benin Raw Export Index Fund`
- Benchmark: `Benin Raw Export Index`
- Fund family: `AFEX`
- Regional subfamily: `WAEX`
- Base currency: `XOF`
- Regional comparison currency: `XOF`
- Continental comparison currency: `USD`

## Methodology Model

- Model: `coastal_export_model`
- Weighting: `trailing 20-year average export tonnage`
- Transport note: Use outbound export tonnage across port-led export flows and official trade statistics.
- Current status: `starter_profile_needs_country_trade_validation`

## Candidate Raw-Material Universe

- `Raw Cotton`
- `Cashew Nuts`
- `Soybeans`
- `Shea Nuts`

## Regional Links

- `Togo`
- `Nigeria`
- `Niger`
- `Burkina Faso`

## Special Note

- Benin should be modeled as both a coastal exporter and a corridor interface for Sahel trade.

## Validation Note

- This package is part of the all-54 AFEX starter library.
- Preferred existing package: `../wasi-upload-waex-all16/benex/benex_fund_characteristics.json`
